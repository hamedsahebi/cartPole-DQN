...
# A version of hidden layer which keeps track of params


class HiddenLayer:
    def __init__(self, M1, M2, f=tf.nn.tanh, use_bias=True):
        self.W = tf.compat.v1.Variable(
            tf.compat.v1.random_normal(shape=(M1, M2)))
        self.params = [self.W]
        self.use_bias = use_bias
        if use_bias:
            self.b = tf.compat.v1.Variable(np.zeros(M2).astype(np.float32))
...
class DQN:
    def __init__(self, D, K, hidden_layer_sizes,
        ...
        self.X = tf.compat.v1.placeholder(
            tf.float32, shape=(None, D), name='X')
        ...
        cost = tf.compat.v1.reduce_sum(
            tf.compat.v1.square(self.G - selected_action_values))
    ...
    def train(self, target_network):
        ...
        targets = [r + self.gamma * next_q for r,
                   next_q in zip(rewards, next_Q)]
...
def play_one(env, model, tmodel, eps, gamma, copy_period):
    ...
    while not done and iters < 2000:
        ...
        model.add_experience(prev_opservation, action, reward, observation)
        model.train(tmodel)
...
def main():
    ...
    for i in range(N):
        ...
        if i % 100 == 0:
            print("episode:", i, "total reward:",
                  totalreward, "eps:", eps, "avg reward (last 100):",
                  totalrewards[max(0, i-100):(i+1)].mean())
